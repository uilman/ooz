...
/// <summary>
/// Decompresses the input file into a byte buffer
/// </summary>
/// <param name="input"></param>
/// <param name="output"></param>
/// <returns></returns>
std::vector<std::uint32_t> DecompressPoEBundle2(byte* input, byte** output, std::size_t& decompressedSize)
{
    /*
    # 010 Editor Template (for PoE bundle.bin files):
uint32 uncompressed_size;
...
for (i = 0; i < head.entry_count; ++i) {
    ...
    *output = new byte[uncompressed_size + SAFE_SPACE];
    ...
    std::uint32_t lastEntry = entry_count - 1;
    constexpr std::size_t BLOCK_SIZE = 262144;
    std::vector<byte> outputChunk(BLOCK_SIZE + SAFE_SPACE);
    for (std::size_t i = 0; i < entry_count; ++i) {
        uint32 unpacked_size = (i == lastEntry) ? (uncompressed_size - (lastEntry * BLOCK_SIZE)) : BLOCK_SIZE;
        
        int outbytesChunk = Kraken_Decompress(input, sizes[i], outputChunk.data(), unpacked_size);

        memcpy(*output + offset, outputChunk.data(), outbytesChunk);
        offset += outbytesChunk;

        input += sizes[i];
    }

    decompressedSize = uncompressed_size;
    return sizes;
}
...
struct InternalFileLocation {
    size_t offset;
    size_t size;
    std::string path;
};
/// <summary>
/// Loops through the bundles to extract and then writes them out from the input
/// </summary>
/// <param name="decompressedInput"></param>
/// <param name="bundles"></param>
void SplitDecompressedPoEBundle2(byte* decompressedInput, const std::vector<InternalFileLocation>& bundles) {
    for (const InternalFileLocation& file : bundles) {
        std::cout << "Extracting file " << file.path << " from offset " << file.offset << std::endl;
        std::string outputPath = file.path;
        std::replace(outputPath.begin(), outputPath.end(), '/', '_');
/*
        std::ofstream output(outputPath, ios::binary);
        output.write((char*)(decompressedInput + file.offset), file.size);
        output.close();
*/


        FILE* f = fopen(outputPath.c_str(), "wb");
        if (!f) error("file open for write error", outputPath.c_str());
        if (fwrite(decompressedInput + file.offset, 1, file.size, f) != file.size)
            error("file write error", outputPath.c_str());
        fclose(f);
    }
}
...
/// <summary>
/// For a given bin file, identify the locations of the files to look for
/// </summary>
/// <param name="binFilePath"></param>
/// <param name="indexBinPath"></param>
/// <param name="pathsToDecompress"></param>
std::vector<InternalFileLocation> processPoECommand(const std::string& binFilePath, const std::string& indexBinPath, const std::vector<std::string>& pathsToDecompress) {
    std::vector<InternalFileLocation> fileInfo;
    //Decompress bin

    //Search for the info

    //Check if the file is in pathsToDecompress
    for(const std::string& path : pathsToDecompress){
        fileInfo.push_back({0,0,path});
    }
    return fileInfo;
}

/// <summary>
/// Extracts a specific file to decompress from the binary file at the specific offset and of a specific size, which will then be saved off locally
/// </summary>
/// <param name="binFile"></param>
/// <param name="pathToDecompress"></param>
/// <param name="offset"></param>
/// <param name="size"></param>
void processPoECommand(byte* input, std::vector<InternalFileLocation>& files) {

    size_t decompressedSize;
    byte* decompressedOutput;
    std::vector<std::size_t> sizes =DecompressPoEBundle2(input,&decompressedOutput,decompressedSize);
    std::cout << "Number of entries: " << sizes.size() << std::endl;
    SplitDecompressedPoEBundle2(decompressedOutput, files);
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2 ||
        ...
        argi >= argc ||  // no files
        (arg_direction == 'g' && ((argc - argi - 1) % 3) != 0) ||  // incorrect amount of files
        (arg_direction == 'p' && (argc - argi) != 4 && (argc - argi) != 2) ||  // incorrect amount of files
        (arg_direction != 'b' && arg_direction != 'g' && arg_direction != 'p' && (argc - argi) > 2) ||  // too many files
        (arg_direction == 't' && (argc - argi) != 2)     // missing argument for verify
        ) {

        std::cout << argc - argi - 1 << std::endl;
        fprintf(stderr, "ooz v7.1 - compressor by Rarten - Modded by WhiteFang\n\n"
            ...
            "Special use case:\n"
            "ooz -g input {[offset] [size] [name]}...\n\n"
            "Decompresses the input file and extracts the specified offsets and sizes into files with the specified name\n"
        );
        return 1;
    }
    ...
    for (; argi < argc; argi++) {
        ...
        if (arg_direction == 'z') {
            ...
            *(uint64*)output = input_size;
            ...
            std::cout << "Decompressing " << std::string(argv[argi]) << "..." << std::endl;
        ...
        else if (arg_direction == 'g') {

            if (arg_dll) {
                LoadLib();
            }

            std::vector<InternalFileLocation> files;
            for (std::size_t i = 0; i < (argc - argi - 1); i+=3) {
                std::cout << argv[argi + i + 1] << ", " << argv[argi + i + 2] << ", " << argv[argi + i + 3] << std::endl;
                files.push_back({ std::stoul(argv[argi + i + 1]),
                                 std::stoul(argv[argi + i + 2]),
                                 argv[argi + i + 3] });
            }
            std::cout << "Processing " << files.size() << std::endl;
            processPoECommand(input, files);
            exit(0);
        }
        else {
            if (arg_dll)
